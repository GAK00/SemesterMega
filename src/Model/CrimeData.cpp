...
bool CrimeData::operator <(const CrimeData & comparedData)
    {
    return generateCompareFactor(*this,comparedData) - generateCompareFactor(comparedData,*this) < -1;
    }
bool CrimeData::operator >(const CrimeData & comparedData)
    {
    return generateCompareFactor(*this,comparedData) - generateCompareFactor(comparedData,*this) >  1;
...
bool CrimeData::operator ==(const CrimeData & comparedData)
    {
    double factor =  generateCompareFactor(*this,comparedData) - generateCompareFactor(comparedData,*this);
    return factor>-1&&factor<1;
    }

double CrimeData::generateCompareFactor(const CrimeData& thisData, const CrimeData & comparedData)
    {
    double comparisonFactor = 0.0;
    compare(thisData.getAllViolent() > comparedData.getAllViolent(), 3, &comparisonFactor);
    compare(thisData.getAllPropertyRates() > comparedData.getAllPropertyRates(), 2.5, &comparisonFactor);
    compare(thisData.getAllViolentRates() > comparedData.getAllViolentRates(), 5, &comparisonFactor);
    compare(thisData.getMurderRates() > comparedData.getMurderRates(), 3, &comparisonFactor);
    double popDif = thisData.getPopulation() - comparedData.getPopulation()*-1;
    comparisonFactor += (popDif / 500000);
    return comparisonFactor;
    }
void CrimeData::compare(bool toEval, double factor, double * comparisonFactor)
    {
    if (toEval)
	{
